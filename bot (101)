...
async def button(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if data == "tasks":
        ...
        # Show main menu
        user = get_user(uid)
        message = format_message(f"{safe_emoji('party', '🎉')} **Welcome to BitcoRise Bot!**\n\n"
            f"{safe_emoji('people', '👥')} **User:** @{update.effective_user.username or 'Anonymous'}\n"
            f"{safe_emoji('chart', '💰')} **Balance:** {user['balance']:.2f}{CURRENCY}\n"
            f"{safe_emoji('fire', '🔥')} **Total Earned:** {user['total_earned']:.2f}{CURRENCY}\n"
            f"{safe_emoji('target', '🎯')} **Daily Limit:** {DAILY_LIMIT}{CURRENCY}\n\n"
            f"{safe_emoji('rocket', '🚀')} **Complete tasks and earn crypto rewards!**\n"
            f"{safe_emoji('gift', '🎁')} **Invite friends and get referral bonuses!**")
        
        keyboard = [
            [InlineKeyboardButton(f"{safe_emoji('target', '🎯')} Earn Tasks", callback_data="tasks")],
            [InlineKeyboardButton(f"{safe_emoji('chart', '💰')} Balance", callback_data="balance"),
             InlineKeyboardButton(f"{safe_emoji('people', '👥')} Referrals", callback_data="referrals")],
