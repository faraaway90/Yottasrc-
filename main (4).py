...
# Fix encoding for VPS deployment
...
async def button(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if data == "tasks":
        ...
        keyboard = [
            [InlineKeyboardButton(f"👍 Like Videos | {TASKS['like']['reward']}{CURRENCY}",
                                  callback_data="like"),
             InlineKeyboardButton(f"💬 Comment Videos | {TASKS['comment']['reward']}{CURRENCY}",
                                  callback_data="comment")],
            [InlineKeyboardButton(f"🔔 Subscribe Channels | {TASKS['subscribe']['reward']}{CURRENCY}",
        ...
        await query.edit_message_text(
            f"💳 **Your Wallet & Statistics**\n\n"
            f"💰 **Current Balance:** {user['balance']}{CURRENCY}\n"
            f"📊 **Total Lifetime Earned:** {user['total_earned']}{CURRENCY}\n"
            f"📈 **Today's Progress:** {user['daily_earned']}{CURRENCY} / {DAILY_LIMIT}{CURRENCY}\n"
            f"🎯 **Remaining Today:** {today_remaining}{CURRENCY}\n"
            f"✅ **Tasks Completed:** {user['tasks_completed']}\n"
            f"👥 **Referrals:** {user['referrals']} (+{user['referrals'] * BONUS_REFERRAL}{CURRENCY})\n"
            f"⏳ **Pending Requests:** {len(pending_requests)} ({pending_amount}{CURRENCY})\n\n"
            f"💸 **Minimum Payout:** {MIN_WITHDRAW}{CURRENCY}\n"
            f"📅 **Available Methods:** FaucetPay & Payeer",
            reply_markup=InlineKeyboardMarkup([[
                InlineKeyboardButton("💸 Request Payout", callback_data="payout"),
                InlineKeyboardButton("💰 Start Tasks", callback_data="tasks")
            ], [
        ...
        if user['balance'] < MIN_WITHDRAW:
            ...
            await query.edit_message_text(
                ...
                f"💸 **Minimum Payout:** {MIN_WITHDRAW}{CURRENCY}\n\n"
                f"**Payment Options:**\n"
                f"💳 **FaucetPay** - Cryptocurrency payments\n"
                f"💎 **Payeer** - Digital wallet payments\n\n"
        ...
        min_amount = MIN_WITHDRAW  # Use main minimum withdrawal amount
    ...
    # Handle task callbacks
        ...
        if f"{uid}_{task_key}" in user_tasks:
            if is_task_completed(uid, task_key):
                ...
                await query.edit_message_text(
                    ...
                    f"🏆 **Total Tasks:** {updated_user['tasks_completed']}\n\n"
                    f"🎯 Keep earning more!",
                    reply_markup=InlineKeyboardMarkup([[
                ...
                if remaining <= 0:
                    # Time is up, show claim button
                    await query.edit_message_text(
                        f"✅ **Task Timer Complete!**\n\n"
                        f"🎯 **Task:** {task['name']}\n"
                        f"💰 **Reward:** {task['reward']}{CURRENCY}\n"
                        f"⏰ **Status:** Ready to claim!\n\n"
                        f"📸 **Before claiming:** Please send a screenshot showing you completed the task, then click claim.\n\n"
                        f"Click the button below to claim your reward! 👇",
                        reply_markup=InlineKeyboardMarkup([[
                            InlineKeyboardButton("🎉 Claim Reward Now!", callback_data=task_key)
                        ], [
                    ...
                    await query.edit_message_text(
                        ...
                        f"Please wait for the timer to complete before claiming your reward.",
                        reply_markup=InlineKeyboardMarkup([[
                            InlineKeyboardButton(f"⏰ Check Again ({format_time(remaining)})", callback_data=task_key)
                        ], [
                            InlineKeyboardButton("🔙 Back to Tasks", callback_data="tasks")
                        ]]), parse_mode='Markdown')
        else:
            ...
            # Add instruction text and completion button
            buttons.append([InlineKeyboardButton("⏰ Check Task Progress", callback_data=task_key)])
            ...
            await query.edit_message_text(
                ...
                f"📌 **Steps to Complete:**\n"
                f"1️⃣ Click the link(s) below to complete the task\n"
                f"2️⃣ Complete the required action (like/comment/subscribe/watch)\n"
                f"3️⃣ Take a screenshot showing you completed it\n"
                f"4️⃣ Wait for {format_time(task['wait'])} timer\n"
                f"5️⃣ Send screenshot and click 'Check Task Progress'\n\n"
                f"🚀 **Timer has started!** Complete the task now! 👇",
                reply_markup=reply_markup, parse_mode='Markdown')
        ...
        # Fast back to menu - just redirect to main menu
        keyboard = [
            [InlineKeyboardButton("💰 Start Tasks", callback_data="tasks"),
             InlineKeyboardButton("💳 Balance", callback_data="balance")],
            [InlineKeyboardButton("💸 Request Payout", callback_data="payout"),
        ...
        await query.edit_message_text(
            ...
            f"📈 **Today's Earnings:** {user['daily_earned']}{CURRENCY} / {DAILY_LIMIT}{CURRENCY}\n"
            f"💸 **Min Payout:** {MIN_WITHDRAW}{CURRENCY}\n\n"
