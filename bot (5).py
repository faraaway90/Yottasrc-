...
# === EMOJI CONSTANTS FOR BETTER HANDLING ===
EMOJIS = {
    'rocket': '🚀',
    'money': '💰',
    'chart': '📊',
            ...
            for j in range(2):
                if i + j < len(task["links"]):
                    ...
                    if task_key == "visit":
                        ...
                        button_text = f"{safe_emoji('eyes', '📺')} Video {link_num}"
...
Ready to start earning? Choose an option below! {safe_emoji('', '👇')}
""")
...
async def button(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if data == "tasks":
        ...
        message = format_message(f"{safe_emoji('target', '🎯')} **Choose a Task to Complete**\n\n"
            ...
            f"Select any task below to start earning! {safe_emoji('', '👇')}")
        await query.edit_message_text(
            message,
